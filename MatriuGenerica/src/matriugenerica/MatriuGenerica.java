...
public abstract class MatriuGenerica<T extends Number> {
    ...
    abstract public <T> Integer[][] suma(Integer[][] matriu1, Integer[][] matriu2);
    abstract public <T> Integer[][] multiplicacio(Integer[][] matriu1, Integer[][] matriu2);
    abstract public <T> T zero();
    abstract public <T> T aleatori();
    ...
    public void sumar(Integer[][] matriu) throws MatriuException {
        ...
        if ((this.matriu.length != matriu.length)
            ...
            throw new MatriuException("La matriu a sumar no és de tamany " + this.matriu.length);
        }
        ...
        for (int i = 0; i < resultat.length; i++) {
            for (int j = 0; j < resultat[i].length; j++) {
                ...
                for (int k = 0; k < this.matriu[0].length; k++) {
                    resultat[i][j] = this.multiplicacio(resultat[i][j], this.matriu[i][k] * matriu[k][j]);
                }
            }
        }
